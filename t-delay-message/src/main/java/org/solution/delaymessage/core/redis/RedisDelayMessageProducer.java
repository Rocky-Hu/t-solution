...
public class RedisDelayMessageProducer implements DelayMessageProducer {
    ...
    private RDelayedQueue<DelayMessage> delayedQueue(String topic) {
        RDelayedQueue<DelayMessage> delayedQueue = delayedQueueRegistry.get(topic);
        if (delayedQueue != null) {
            return delayedQueue;
        } else {
            RBlockingQueue<DelayMessage> blockingQueue = redissonClient.getBlockingQueue(topic);
            delayedQueue = redissonClient.getDelayedQueue(blockingQueue);
            delayedQueueRegistry.putIfAbsent(topic, delayedQueue);
            return delayedQueue;
        }
    }
